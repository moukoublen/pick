...
func TestReadme(t *testing.T) {
	...
	// Silent API
	t.Run("basic silent api", func(t *testing.T) {
		assert := tst.AssertEqualFn(t)

		assert(p1.Silent().Int32("item.one"), int32(1))
		assert(p1.Silent().Int32("non-existing"), int32(0))
	...
	t.Run("generics", func(t *testing.T) {
		...
		m := p1.Silent()
		assert(SilentGet[string](m, "item.three[1]"), "2")

		assert2(Path[string](p1, Field("item"), Field("three"), Index(1)))("2", nil)
		assert(SilentPath[float32](m, Field("item"), Field("one")), float32(1))
	})
