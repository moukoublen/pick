...
func TestUsingMixedTypesMapUsingFloat64API(t *testing.T) {
	...
	tests := []PickerTestCase{
		...
		{
			AccessFn:      p.Silent().Float64Slice,
			Selector:      "pointerMapStringAny.float64Slice",
			ExpectedValue: []float64{0.1, 0.2, 0.3, 0.4},
			ErrorAsserter: tst.NoError,
		},
...
func TestWithMixedTypesMapUsingFloat32API(t *testing.T) {
	...
	tests := []PickerTestCase{
		...
		{
			AccessFn:      p.Silent().Float32Slice,
			Selector:      "pointerMapStringAny.float64Slice",
			ExpectedValue: []float32{0.1, 0.2, 0.3, 0.4},
			ErrorAsserter: tst.NoError,
		},
...
func TestWithMixedTypesMapUsingDurationAPI(t *testing.T) {
	...
	tests := []PickerTestCase{
		...
		{
			AccessFn:      p.Silent().DurationSlice,
			Selector:      "durations.slice",
			ExpectedValue: []time.Duration{5 * time.Second, 6 * time.Second, 7 * time.Second},
			ErrorAsserter: tst.NoError,
		},
	...
	tests := []PickerTestCase{
		...
		},
		{
			AccessFn: func(selector string) []string {
				return SilentMap(p.Silent(), selector, func(a SilentAPI) (string, error) { return a.String("id"), nil })
			},
			Selector:      "near_earth_objects.2023-01-01",
			ExpectedValue: []string{"2154347", "2385186", "2453309", "3683468", "3703782", "3720918", "3767936", "3792438", "3824981", "3836251", "3837605", "3959234", "3986848", "54104550", "54105994", "54166175", "54202993", "54290862", "54335607", "54337027", "54337425", "54340039", "54341664"},
			ErrorAsserter: tst.NoError,
		...
		},
		{
			AccessFn: func(_ string) []string {
				return SilentFlatMap(p.Silent(), "near_earth_objects.2023-01-01", func(a SilentAPI) ([]string, error) {
					return SilentMap(a, "close_approach_data", func(a SilentAPI) (string, error) {
...
func TestMapSilent(t *testing.T) {
	t.Parallel()
	file := loadTestData(t, "nasa.json")
	...
	t.Run("happy path", func(t *testing.T) {
		t.Parallel()
		errSink := &ErrorsSink{}
		itemsSlice := SilentMap(p.Silent(errSink), "near_earth_objects.2023-01-07", func(sm SilentAPI) (Item, error) {
			return Item{
				Name:   sm.String("name"),
	...
	t.Run("gather errors", func(t *testing.T) {
		t.Parallel()
		errSink := &ErrorsSink{}
		_ = SilentMap(p.Silent(errSink), "near_earth_objects.2023-01-07", func(sm SilentAPI) (Item, error) {
			return Item{
				Name:   sm.String("name"),
...
func TestEach(t *testing.T) {
	...
	t.Run("EachM happy path", func(t *testing.T) {
		t.Parallel()
		errSink := &ErrorsSink{}
		SilentEach(p.Silent(errSink), "near_earth_objects.2023-01-07", func(index int, a SilentAPI, length int) error {
			tst.AssertEqual(t, length, 17)
			if index == 4 {
				s := a.String("name")
				tst.AssertEqual(t, s, "(2006 HJ18)")
...
func TestHTTP(t *testing.T) {
	t.Run("request", func(t *testing.T) {
		...
		require.NoError(t, err)
		require.Equal(t, 1, p.Silent().Int("one"))
	})
	...
	t.Run("response", func(t *testing.T) {
		...
		require.NoError(t, err)
		require.Equal(t, "two", p.Silent().String("one"))
	})
